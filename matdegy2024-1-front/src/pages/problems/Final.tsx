...
const Codeforces = () => {
  ...
  const checkAnswer = async () => {
    ...
    setShowAlert(true);
    axios
      .get("/api/submit/final?submissionID=" + answer)
      .then((res) => {
        if (res.data.correct) {
          setIsCorrect(true);
  ...
  useEffect(() => {
    axios.get("/api/submit/road?answer=bug_c4mp_messssi");
  }, []);
  useEffect(() => {
    if (isCorrect === null) {
  ...
  return (
    <ProblemEachField>
      ...
      <Text>
        유림이는{" "}
        <Link to="https://codeforces.com/profile/kidw0124" target="_blank">
          동우
        </Link>
        네 집에서 흥미로운 무언가를 발견했다. <br />
      ...
      <Text>
        ...
        에서 Final 문제를 Accepted를 띄운 뒤, <br />
        제출한 코드의 번호를 입력하면 된다.
      </Text>
